...
#intersection
my %peak2len;
open(IN,"cortex_ac_v5_all_bypeak_enh_140723_intersect_ccemerged.txt") || die $!;
while(<IN>) {
	tr/\r\n//d;
	my @array=split/\t/;
	$peak2len{$array[3]}{$clade2name{$array[7]}}+=$array[8];
}
close IN;

#result
open(OUT,">cortex_ac_v5_all_bypeak_enh_140723_intersect_ccemerged_lensummary.txt") || die $!;
